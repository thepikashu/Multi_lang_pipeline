...
st.set_page_config(page_title="Yashasvee's Multilingual NLP Engine", layout="wide")
...
if st.button("Run Intelligence Pipeline"):
    if user_input.strip():
        with st.spinner("Analyzing linguistic patterns..."):
            ...
            tab1, tab2 = st.tabs(["Noun Phrases", "Tokens & Lemmas"])
            with tab1:
                st.write(", ".join(results['basic']['noun_phrases']))
            with tab2:
                st.write(results['basic']['lemmas'])
    else:
